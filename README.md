...
1. 调整好微信窗口、联系人分栏和输入区，在菜单栏 **J → 校准消息识别区域…** 打开窗口截图。
2. 拖动框选消息区，包含双方头像、昵称和气泡，排除联系人列表、标题、公告和输入框。拖动四条边可微调；重新拖动内部可重画。
3. 点击 **预览识别**。绿框表示已判定归属的气泡，橙框表示未确认文字；文字标签区分「对方／我／未确认」。预览仅本地 OCR，不调用判断或生成服务。
4. 点击 **确认并启用**，回到微信后开始分析。取消不会保存本次修改；菜单栏 **恢复自动识别区域** 可退出实验模式。

选区保存到现有用户 env 的 `JEV_MESSAGE_REGION`，值是窗口宽高及选区 x/y/宽/高（窗口点坐标），保留其他配置和注释、文件权限 600。每次启动必须重新确认保存的选区；窗口整体移动无需重画，窗口尺寸或窗口身份改变后暂停并要求重新校准。**拖动内部列表、改变输入区高度、切换带公告的聊天后，请主动重新校准**；当前不保证自动识别这些内部布局变化。选区上方独立读取聊天标题，标题为空时不分析。

校准模式按气泡表面分组普通文字：同一气泡内合并多行，独立气泡不因距离近而合并；不按字符数或窗口高度删除短句。左右归属基于选区内气泡位置和同侧对齐，无法判定的文字只显示，不进入模型上下文或自动回复目标。图片、引用、复杂主题和特殊昵称尚未充分支持，可能仍为「未确认」；这不是通用视觉理解模型。

**实验模式暂不提供填入，请使用复制。** 消息选区不是输入框授权，不能复用已知可能错位的视觉输入框定位。现有自动模式维持原行为。手动模式每次重新 OCR，不复用空帧的旧消息，不新增哈希；因此性能与自动模式不同。

验证：离线回归覆盖短句、相邻气泡、多行文本、宽列表下的左右归属、深色合成气泡、选区边界、配置权限及空帧不复用。原生窗口已验证预览/保存/取消与恢复入口；截图复测覆盖宽窄列表和私聊。尚未完成真实微信消息到在线模型候选的全流程验收，也未验证所有微信版本及主题。个人截图不随代码提交。
