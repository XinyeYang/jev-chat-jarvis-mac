...
![识别效果实拍：微信消息实时框选标注，面板按话术分组给出候选并带本地排序概率（图中「狗头军师」置顶）](docs/recognition-effect.png)

聊天标题支持单字和较短的联系人名称；识别到聊天标题变化时，即使最后一条消息相同，也会重新分析。

## 手动校准消息区域（fork 实验分支）

联系人列表可拖动时，固定比例的自动区域可能漏消息或读到列表摘要。手动校准按你框选的真实气泡整理消息，**识别准确度大幅提高**：短句不再被当杂字丢掉、多行正文完整合并、左右归属更准。

打开 app 时，点击右上角手动校准图标即可进入：

![打开 app 时，点击右上角校准图标以手动校准](docs/first-run-hint.png)

同一界面拖框选消息识别区域（绿框）与输入区域（蓝框），预览通过后一次保存：

![校准区域：绿框罩住整个消息流，蓝框罩住输入区，预览识别通过后「确认并启用」](docs/calibration-window.png)
