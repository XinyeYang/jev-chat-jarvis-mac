...
4. 点击 **确认并启用**，回到微信后开始分析。需要填入时，再打开 **J → 校准输入区域…**，框选消息区下方的完整文字编辑区，排除底部工具栏和发送按钮，预览后确认。
5. 取消不会保存本次修改；菜单栏 **恢复自动识别区域** 可退出实验模式。两个校准窗口共用主面板/模型设置的磨砂背景、圆角卡片、配色和按钮样式。

消息区和输入区分别保存到现有用户 env 的 `JEV_MESSAGE_REGION`、`JEV_INPUT_REGION`，值是窗口宽高及选区 x/y/宽/高（窗口点坐标），保留其他配置和注释、文件权限 600。每次启动必须重新确认保存的消息区和输入区；重新确认消息区后，输入区也需要重新确认；窗口整体移动无需重画，窗口尺寸或窗口身份改变后暂停并要求重新校准。**拖动内部列表、改变输入区高度、切换带公告的聊天后，请主动重新校准**；当前不保证自动识别这些内部布局变化。选区上方独立读取聊天标题，标题为空时不分析。
...
**输入区未单独确认时不允许填入，可使用复制。** 手动输入区仅在明确点击「填入」时使用：检查窗口与会话、读取整个编辑区的草稿（不跳过底部），点击获得焦点后再次检查草稿，再输入并 OCR 读回。不会清空草稿、发送消息或使用剪贴板。手动坐标不走自动输入区定位；用户改变内部布局后必须重新校准，不能保证自动发现分栏/输入区高度变化。现有自动模式维持原行为。手动模式每次重新 OCR，不复用空帧的旧消息，不新增哈希；因此性能与自动模式不同。
